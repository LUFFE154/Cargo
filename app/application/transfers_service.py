...
class TransferService:
    ...
    async def search_transfers(self, *, owner_id: UUID, query: str) -> list[Transfer]:
        async with self._uow as uow:
            return list(await uow.transfers.search(owner_id, query))

    async def _create_unique_transfer_code(self, uow: UnitOfWork, owner_id: UUID) -> str:
        for _ in range(20):
    ...
    def _build_stored_name(self, owner_id: UUID, original_name: str) -> str:
